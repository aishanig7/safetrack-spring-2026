...
volatile bool receivedFlag = false;  // flag to indicate packet received (IRQ)
...
void setup() {
  ...
  if (st != RADIOLIB_ERR_NONE) {
    Serial.println(
        "radio.begin() failed. Check wiring, power, constructor ordering.");
    while (true) {
      delay(1000);
    }
  }
...
void loop() {
  const uint8_t selfId = NodeIdentity::getNodeId();
  ...
  if (receivedFlag) {
    ...
    if (pkt->type == PKT_GPS &&
        (pkt->dst == selfId || pkt->dst == MESH_BROADCAST)) {
      float rxLat, rxLng;
      uint8_t rxNode;
